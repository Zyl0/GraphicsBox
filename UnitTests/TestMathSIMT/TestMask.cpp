    ...
    )
{
    ...
    SECTION("Bitwise operators")
    {
        ...
        MaskType mand = m1 & m2;
        REQUIRE(mand.bits[0] == 2); // 010

        MaskType mor = m1 | m2;
        REQUIRE(mor.bits[0] == 7); // 111

        MaskType mxor = m1 ^ m2;
        REQUIRE(mxor.bits[0] == 5); // 101

        MaskType minv = ~m1;
        Type expected_minv_bits = (~Type(3)) & MaskType::FullBitMask();
        REQUIRE(minv.bits[0] == expected_minv_bits);

        MaskType mnot = !m1;
        REQUIRE(mnot.bits[0] == expected_minv_bits);
    }
    ...
    {
        MaskType m_full({MaskType::FullBitMask()});
