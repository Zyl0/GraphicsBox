...
namespace Math::Simt
{
    ...
    struct Transform4 : Matrix4<DataType, ThreadCount>
    {
        ...
        {        
            Scalar<DataType, ThreadCount> c = Cos(t);
            Scalar<DataType, ThreadCount> s = Sin(t);
            Scalar<DataType, ThreadCount> d = 1 - c;
            ...
            return Transform4(
                c +  x * axis.x,    axay - s * axis.z,  axaz + s * axis.y,  0,
                axay + s * axis.z,  c + y * axis.y,     ayaz - s * axis.x,  0,
                axaz - s * axis.y,  ayaz + s * axis.x,  c + z * axis.x,     0
            );
        }
        ...
        INLINE static Transform4 Reflection(Scalar<DataType, ThreadCount> axis)
        {
            ...
            Scalar<DataType, ThreadCount> z = axis.z * -2;
            ...
            return Transform4(
                x * axis.x + 1,    axay,           axaz,           0,
                axay,           y * axis.y + 1,    ayaz,           0,
                axaz,           ayaz,           z * axis.z + 1,    0
            );
        }
        
        INLINE static Transform4 Reflection(const Plane<DataType, ThreadCount>& f)
        {
            Scalar<DataType, ThreadCount> x = f.x * -2;
            Scalar<DataType, ThreadCount> y = f.y * -2;
            Scalar<DataType, ThreadCount> z = f.z * -2;
            ...
            return Transform4(
                x * f.x + 1,    nxny,           nxnz,           x * f.w,
                nxny,           y * f.y + 1,    nynz,           y * f.w,
                nxnz,           nynz,           z * f.z + 1,    z * f.w
            );
        }
        ...
        INLINE static Transform4 Perspective(Scalar<DataType, ThreadCount> fieldOfView, Scalar<DataType, ThreadCount> aspectRatio, Scalar<DataType, ThreadCount> zNear, Scalar<DataType, ThreadCount> zFar)
        {
            ...
            return reinterpret_cast<Transform4>(
                Matrix4<DataType, ThreadCount>(
    ...
    INLINE Transform4<DataType, ThreadCount> Inverse(const Transform4<DataType, ThreadCount> &H)
    {
        ...
        Scalar<DataType, ThreadCount> invDet = 1 / Det;
